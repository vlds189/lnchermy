...
- `version_list_section()`: вместо списка кнопок — переиспользуемый
  `super::selector::selector()` (`docs/ui/selector.md`): ComboBox с выбором
  версии, searchbar (фильтр по подстроке, колбэк-фильтр в `main_view`),
  кнопка 🗑 удаления у каждой строки. При `LaunchStatus::Running` передаётся
  `enabled: false` — опции grey-out (disabled-виджеты в egui не ховерятся,
  нет подсветки и курсора). Индексы селектора мапятся на
  `installed_versions`; выбор версии сбрасывает `LaunchStatus::Idle`.
