...
- `render()` — сборка панелей: `Panel::top` (заголовок + версия), `Panel::left`
  ...
  возврат к выбору версии, и ⚙ Settings; все иконки — SVG из
...
- Сайдбар: кнопка геймпада выходит из настроек (через
  `settings_view::mark_leaving()`, чтобы буферы ника/URL пере-засеялись)
  и ставит `state.request_picker_open` — launch-строка в том же кадре
  открывает picker (`SetOpenCommand::Bool(true)`).
